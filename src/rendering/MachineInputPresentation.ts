...
import { POINTER_DRAG_THRESHOLD_PX } from '@/config/input-balance.ts'
...
import { isTerrainMesh, terrainMeshPickPredicate } from '@rendering/resolveTerrainMesh.ts'
import {
  MachineId,
  SelectedEntityKind,
  type MachineCommand,
...
export class MachineInputPresentation {
  ...
  attach(scene: Scene, game: Game): void {
    ...
    this.pointerObserver = scene.onPointerObservable.add((pointerInfo) => {
      ...
      if (pointerInfo.type === PointerEventTypes.POINTERDOWN) {
        const pick = this.resolvePointerDownPick(pointerInfo, event)
        this.pointerStart = {
          x: event.clientX,
          y: event.clientY,
          button: event.button,
      ...
      if (pointerInfo.type === PointerEventTypes.POINTERMOVE) {
        ...
        if (dragDistance > POINTER_DRAG_THRESHOLD_PX) {
          start.dragged = true
        }
        return
      }
      ...
      if (!start || start.button !== event.button || start.dragged) {
        return
      }

      if (event.button === 0) {
        this.handleLeftClick(start.mesh, start.point, start.x, start.y)
        return
      }
  ...
  private handleLeftClick(
    ...
    const resolved =
      mesh !== null
        ? { mesh, point }
        : this.resolveLeftClickPick(clientX, clientY)

    const machineId = resolved.mesh
      ? this.resolveMachineId(resolved.mesh)
      : null
    if (machineId) {
    ...
    const fieldId = resolved.mesh
      ? this.resolveFieldId(resolved.mesh)
      : resolved.point
        ? this.resolveFieldAtPosition({
            x: resolved.point.x,
            z: resolved.point.z,
          })
        : null
    if (fieldId) {
      this.game?.selectField(fieldId)
      return
    }

    this.game?.clearMapSelection()
  }
  ...
  private handleRightClick(
    ...
    if (!mesh || !isTerrainMesh(mesh) || !point) {
      return
    }
  ...
  private resolvePointerDownPick(
    pointerInfo: { pickInfo?: PickingInfo | null },
    event: PointerEvent,
  ): { mesh: AbstractMesh | null; point: Vector3 | null } {
    if (event.button === 0) {
      return this.resolveLeftClickPick(event.clientX, event.clientY)
    }

    if (event.button === 2) {
      return this.resolvePick(pointerInfo, event)
    }
  ...
  private resolveLeftClickPick(
    ...
    const fieldPick = this.pickFromClient(clientX, clientY, (mesh) =>
      this.isFieldMesh(mesh),
    )
    if (fieldPick.mesh) {
      return fieldPick
    }

    const terrainPick = this.pickFromClient(clientX, clientY, terrainMeshPickPredicate)
    if (terrainPick.mesh) {
      return terrainPick
    }

    return this.pickFromClient(clientX, clientY)
  }
  ...
  private resolvePick(
    ...
    if (machineSelected && event.button === 2) {
      ...
      const fieldPick = this.pickFromClient(
        event.clientX,
        event.clientY,
        (mesh) => this.isFieldMesh(mesh),
      )
      if (fieldPick.mesh) {
        return fieldPick
      }

      const terrainPick = this.pickFromClient(
        event.clientX,
        event.clientY,
        terrainMeshPickPredicate,
      )
      if (terrainPick.mesh) {
        return terrainPick
      }

      const broadPick = this.pickFromClient(
        event.clientX,
        event.clientY,
        (mesh) =>
          !this.isMachineMesh(mesh) &&
  ...
  private resolveFieldAtPosition(position: {
    ...
    for (const [fieldId, center] of Object.entries(getFieldPositions())) {
